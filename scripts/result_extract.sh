...
output_file=${SPIPE_ROOT:-.}/actual_result.csv

echo "jobid,name,model_size,mbs,gbs,nnode,seq,tflops,elapsed_time_ms,gpu_latency" > "$output_file"
...
for file in slurm-*.out; do
    ...
    mbs=$(echo "$config_block"       | grep -m1 "MBS="            | cut -d'=' -f2)
    gbs=$(echo "$config_block"       | grep -m1 "GBS="            | cut -d'=' -f2)
    ...
    echo "$jobid,$name,$model_size,$mbs,$gbs,$nnode,$seq,$tflops,$elapsed,$gpu_latency" >> "$output_file"
done
