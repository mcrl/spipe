...
echo "jobid,name,model_size,mubs,mbs,nnode,seq,tflops,elapsed_time_ms,gpu_latency" > "$output_file"
...
for file in slurm-*.out; do
    ...
    echo "$jobid,$name,$model_size,$mubs,$mbs,$nnode,$seq,$tflops,$elapsed,$gpu_latency" >> "$output_file"
done
