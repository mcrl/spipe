...
def spipe_schedule(
    ...
    **kwargs,
    ...
    assert isinstance(
        model, list
    ), "SPipe expected model chunking by stage"
    assert isinstance(
        data_iterator, list
    ...
    sync_ckpt_comm = get_args().spipe_sync_ckpt_communication
    if not forward_only:
        ckpt_send_recv_schedule = CkptSendRecvSchedule(
            num_microbatches=num_microbatches, use_sync=sync_ckpt_comm
        )
    use_ckpt_batch_p2p_comm = not get_args().spipe_ckpt_p2p

    # Data structures for training
    forward_data_store = []
    recvs: List[Tuple[torch.Tensor, List[Work]]] = []
    ckpt_recvs: List[List[Tuple[torch.Tensor, Work]]] = [[] for _ in range(mpu.get_spipe_backward_virtual_size())]
    ...
    for fwd_stage_id in range(mpu.get_spipe_forward_virtual_size()):
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("prefetch")):
            ...
            if not (
                forward_only
                and fwd_stage_id == mpu.get_spipe_forward_virtual_size() - 1
            ):
                model[fwd_stage_id + 1].spipe_fetch(non_blocking=True)
                tag = "prefetch:" + (
                    f"f{fwd_stage_id + 1}"
                    if fwd_stage_id + 1 < mpu.get_spipe_forward_virtual_size()
                    else f"b{mpu.get_spipe_backward_virtual_size() - 1}"
                )
        ...
        for m_i in range(num_microbatches):
            ...
            with torch.cuda.stream(get_thunder_cuda_manager().Stream("compute")):
                ...
                # NOTE (SPipe) Original FWD should not create computation graph for two reasons
    ...
    for bwd_stage_id in range(
        ...
        assert (
            hasattr(model[-bwd_stage_id - 1], "spipe_backward_stage_id")
            and getattr(model[-bwd_stage_id - 1], "spipe_backward_stage_id")
            == bwd_stage_id
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("prefetch")):
            if (
                get_thunder_cuda_manager().wait_event(
                    free_event_queries.pop(
                        ...
                        if bwd_stage_id == mpu.get_spipe_backward_virtual_size() - 1
            ...
            if not bwd_stage_id == 0:
                model[-bwd_stage_id].spipe_fetch(non_blocking=True)
                prefetch_next = get_thunder_cuda_manager().Event(
                    "prefetch",
                    "compute",
        ...
        for m_i in range(num_microbatches):
            ...
            with torch.cuda.stream(get_thunder_cuda_manager().Stream("compute")):
                ...
                output_tensor = forward_step(
                    forward_step_func,
                    data_iterator[
                        bwd_stage_id + mpu.get_spipe_forward_virtual_size()
                    ],
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("free")):
            ...
            if _is_cpu_optimizer(bwd_stage_id):
                model[-bwd_stage_id - 1].spipe_free()
            free_curr = get_thunder_cuda_manager().Event(
                "free",
        ...
        if offload_grad_after_bwd_stage:
            with torch.cuda.stream(get_thunder_cuda_manager().Stream("offload")):
                ...
                # offload grads
                if _is_cpu_optimizer(bwd_stage_id):
                    model[-bwd_stage_id - 1].spipe_offload_grad(non_blocking=True)
                ...
                if optimize_after_bwd_stage:
                    ...
                    if not optimizer.is_cpu_optimizer(bwd_stage_id):
                        model[-bwd_stage_id - 1].spipe_offload(non_blocking=True)
                        model[-bwd_stage_id - 1].spipe_free()
                ...
                model[-bwd_stage_id - 1].spipe_free_grad()
