...
from megatron.spipe.generic import ContextManagers
...
def mobius_no_prefetch_schedule(
    *,
    forward_step_func,
    ...
    **kwargs,
    ...
    # TODO (SPipe) Recomputation implementation currently suffers redundant saving of input/output tensors.
    recompute = get_args().spipe_recompute_activations

    assert isinstance(model, list), "SPipe expected model chunking by stage"
    assert isinstance(
        data_iterator, list
    ), "SPipe expected each model chunk to have a data iterator"

    # TODO (SPipe) disable async grad reductions?
    ...
    if mpu.get_spipe_forward_virtual_rank() is not None:
        warnings.warn(
            "SPipe forward virtual rank is not None on scheule entry. There may be a bug."
        )
    if mpu.get_spipe_backward_virtual_rank() is not None:
        warnings.warn(
            "SPipe backward virtual rank is not None on scheule entry. There may be a bug."
        )
    ...
    if model_type == ModelType.encoder_and_decoder:
        raise RuntimeError("SPipe is not supported with an encoder and decoder model.")

    if decoder_seq_length is not None and decoder_seq_length != tensor_shape[0]:
        raise RuntimeError(
            "SPipe is not supported with a different decoder sequence length."
        )

    offload_grad_after_bwd_stage = get_args().spipe_overlap_offload_grad
    optimize_after_bwd_stage = offload_grad_after_bwd_stage and get_args().spipe_stage_optimizer
    if get_args().spipe_stage_optimizer:
        assert "spipe_stage_optimizer" in kwargs
        optimizer = kwargs["spipe_stage_optimizer"]
    ...
    # NOTE (SPipe) forward_step() in megatron/core/pipeline_parallel/schedules.py has some additional logic to compute loss using output tensor of the last pipeline stage, which is not captured by spipe output tensors. This is a temporary workaround to capture the loss tensor, and a better solution may exist.
    if (
        not forward_only
        and not recompute
    ...
    #     model[0].spipe_fetch(non_blocking=nonblocking_prefetch)
    ...
    for fwd_stage_id in range(mpu.get_spipe_forward_virtual_size()):
        if _DEBUG_SCHEDULE:
            spipe_print(f"Start fwd stage {fwd_stage_id}")
        mpu.set_spipe_forward_virtual_rank(fwd_stage_id)

        assert (
            hasattr(model[fwd_stage_id], "spipe_forward_stage_id")
            and getattr(model[fwd_stage_id], "spipe_forward_stage_id") == fwd_stage_id
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("prefetch")):
            ...
            model[fwd_stage_id].spipe_fetch(non_blocking=False)
            tag = "prefetch:" + f"f{fwd_stage_id}"
            prefetch_next = get_thunder_cuda_manager().Event(
                "prefetch",
                "compute",
                tag=tag,
                post_wait_fn=lambda fwd_stage_id=fwd_stage_id: set_module_spipe_status(
                    model[fwd_stage_id], SPipeParamStatus.GPU
                ),
            )
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("free")):
            ...
            if not fwd_stage_id == mpu.get_spipe_forward_virtual_size() - 1:
                model[fwd_stage_id].spipe_free()
    ...
    for bwd_stage_id in range(
        ...
        if _DEBUG_SCHEDULE:
            spipe_print(f"Start bwd stage {bwd_stage_id}")
        mpu.set_spipe_backward_virtual_rank(bwd_stage_id)

        assert (
            hasattr(model[bwd_stage_id], "spipe_backward_stage_id")
            and getattr(model[bwd_stage_id], "spipe_backward_stage_id") == bwd_stage_id
        ...
        with torch.cuda.stream(get_thunder_cuda_manager().Stream("prefetch")):
            ...
            if not bwd_stage_id == mpu.get_spipe_backward_virtual_size() - 1:
                if (
            ...
            model[bwd_stage_id].spipe_fetch(non_blocking=False)
            prefetch_next = get_thunder_cuda_manager().Event(
                "prefetch",
                "compute",
                tag="prefetch:" + f"b{bwd_stage_id}",
                post_wait_fn=lambda bwd_stage_id=bwd_stage_id: set_module_spipe_status(
                    model[bwd_stage_id], SPipeParamStatus.GPU
                ),
            )
            if get_thunder_cuda_manager().record_event(prefetch_next) == -1:
                raise RuntimeError("record_event failed")
        ...
        for m_i in range(num_microbatches):
            ...
            input_tensor_ckpt = (
                model[bwd_stage_id].module[0].spipe_input_tensors.popleft()
            )
            ...
            with torch.cuda.stream(get_thunder_cuda_manager().Stream("compute")):
                if recompute:
                    ...
                    output_tensor = (
                        model[bwd_stage_id].module[-1].spipe_output_tensors.popleft()
                    )
                    # NOTE (SPipe) Although we can make conditional code with above code line that handles output tensor, we do it this way to pop the output tensor from the last pipeline stage, just to be sure that those tensors should not be accesssed from somewhere else.
        ...
        if offload_grad_after_bwd_stage:
            with torch.cuda.stream(get_thunder_cuda_manager().Stream("offload")):
                ...
                # offload grads
                if _is_cpu_optimizer(bwd_stage_id):
                    model[bwd_stage_id].spipe_offload_grad(non_blocking=True)
                ...
                if optimize_after_bwd_stage:
                    ...
                    if not optimizer.is_cpu_optimizer(bwd_stage_id):
                        model[bwd_stage_id].spipe_offload(non_blocking=True)
                ...
                model[bwd_stage_id].spipe_free_grad()
