...
class SpiralPhaseList(MegatronModule):
    ...
    def __init__(
        ...
        Must be initialized under SpiralInitContext in order to have spiral attributes set properly, and have spiral functions that propagate to the wrapped modules.

        Modules in `modules` must be initialized in this ctor, in order to correctly walk through the recursive init process of SpiralInitContext.
        ...
        for module in self.module_list:
            if save_input_tensors:
                module.spiral_input_tensors = deque(maxlen=get_num_microbatches())
            else:
                module.spiral_input_tensors = None
            if save_output_tensors:
                module.spiral_output_tensors = deque(maxlen=get_num_microbatches())
            else:
                module.spiral_output_tensors = None
    ...
    # NOTE (SpiralPipe) Do not rename this function
    ...
    def _set_phase_input_tensor(self, phase_id, input_tensor):
        ...
        # TODO (SpiralPipe) Current POC saves all input tensors, where it can be optimized to save only the necessary input tensor ckpts
        if hasattr(self.module_list[phase_id], "spiral_input_tensors") and isinstance(
            getattr(self.module_list[phase_id], "spiral_input_tensors"), deque
        ):
            self.module_list[phase_id].spiral_input_tensors.append(input_tensor[0])

    def _set_phase_output_tensor(self, phase_id, output_tensor):
        if hasattr(self.module_list[phase_id], "spiral_output_tensors") and isinstance(
            getattr(self.module_list[phase_id], "spiral_output_tensors"), deque
        ):
            self.module_list[phase_id].spiral_output_tensors.append(output_tensor)

    def empty_input_tensors(self):
        for module in self.module_list:
            if hasattr(module, "spiral_input_tensors") and isinstance(
                getattr(module, "spiral_input_tensors"), deque
            ):
                module.spiral_input_tensors.clear()

    def empty_output_tensors(self):
        for module in self.module_list:
            if hasattr(module, "spiral_output_tensors") and isinstance(
                getattr(module, "spiral_output_tensors"), deque
            ):
                module.spiral_output_tensors.clear()
